...
| Scenario | `data/src/scenario.ts` | `createCaveScenario(seed)`, `createPlayerDemoWorld(seed)`, `walkDistances(world, layer, from)` (BFS, string keys `"x,y"`), `CAVE_SPAWN_MIN_STEPS`/`MAX_STEPS` = 22/40, `CAVE_STARTER_SPECIES` (34-species prey pool; direct ask: "I want starting cave to be a random pack of prey... let's make a decent wide pool"). Chamber is painted around an underground water pocket: sunbeams, food, flora, a 4-member herd of one species rolled from the pool — find it by `herdId?.endsWith("-herd")`, never hardcode a species. |
