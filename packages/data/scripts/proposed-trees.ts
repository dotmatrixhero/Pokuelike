...
export const PROPOSED_TREES: Record<string, ProposedMove> = {
  ...
  twineedle: {
    ...
    learners: ["beedrill"],
    fantasy:
      "Two strikes, one behind the other, from a thing that is mostly needles. A Beedrill does not grapple; it commutes. It arrives, stabs twice, and is gone before you have turned around. It is a poison delivery system with wings. PILOT TREE for the two-lane structure: each branch runs two parallel filler lanes with a notable in each, and nothing is locked out — you can walk both lanes if you are willing to spend the points.",
    tree: tree([
      // ================= AGGRESSION: the flurry =================
      { id: "third_needle", name: "Third Needle", cost: 1, leaning: "aggression", delta: { hits: { min: 2, max: 3 } },
        note: "OPENER. The branch splits here into a raw lane and a reserve lane. Each stab rolls poison independently, so a third stab is a third chance." },
      // --- Lane P: raw. More needles, harder.
      { id: "quicker_draw", name: "Quicker Draw", cost: 1, leaning: "aggression",
        prerequisitesAnyOf: [["third_needle"], ["blur_of_needles"], ["venom_mark"]], delta: { cooldownTicks: -1 } },
      ...
      { id: "fourth_needle", name: "Fourth Needle", cost: 2, prerequisites: ["barbed"], leaning: "aggression", delta: { hits: { min: 3, max: 4 } },
        note: "LANE P NOTABLE. Four independent poison rolls is a near-certainty — this lane's product is reliability." },
      { id: "thin_point", name: "Thin Point", cost: 1, prerequisites: ["fourth_needle"], leaning: "aggression",
        ...
        note: "It closes the last two tiles itself before the first needle lands." },
      // --- Lane R: reserve. Fewer, bigger, paid for in PP.
      { id: "venom_sacs", name: "Venom Sacs", cost: 1, leaning: "aggression",
        prerequisitesAnyOf: [["third_needle"], ["blur_of_needles"], ["venom_mark"]],
        delta: { maxPPBonus: 8 }, needsPrimitive: PPC,
        note: "LANE R entry. +8 on a 20 pool is +40% — worth it only if you are walking this lane to Pincushion." },
      { id: "measured_strikes", name: "Measured Strikes", cost: 1, prerequisites: ["venom_sacs"], leaning: "aggression",
        ...
        note: "Slower, more deliberate strikes — the opposite tempo to Lane P." },
      { id: "pincushion", name: "Pincushion", cost: 2, prerequisites: ["measured_strikes"], leaning: "aggression",
        delta: { statusSeverity: 3, statusChance: 0.3, defensePenetration: 0.25, ppCost: 3 }, needsPrimitive: PPC,
        note: "LANE R NOTABLE. Deliberately NOT more needles — that is Lane P's answer. Two strikes that go deep and leave venom that actually works. 3 PP a use: six flurries on the base pool, nine if you took Venom Sacs." },
      ...
      // --- Both lanes end here.
      { id: "hollow_points", name: "Hollow Points", cost: 2, leaning: "aggression",
        prerequisitesAnyOf: [["thin_point"], ["conserving_draw"]],
        delta: { statusSpreads: true, hitsArea: true, shape: { kind: "cone", length: 2, width: 2 } },
        note: "KEYSTONE. Reachable from either lane. Enough venom in one flurry that it comes off on whatever is standing nearby." },
      ...
      { id: "hit_and_gone", name: "Hit and Gone", cost: 1, leaning: "boldness",
        ...
        note: "OPENER. Splits into a low fast lane and a high wide one. The USER retreats after landing — shipped primitive, used on the attacker, which almost nothing does." },
      // --- Lane L: low and close.
      { id: "wide_approach", name: "Wide Approach", cost: 1, leaning: "boldness",
        prerequisitesAnyOf: [["hit_and_gone"], ["blur_of_needles"], ["ambush_hive"]], delta: { range: { max: 2 } } },
      ...
      { id: "circling_high", name: "Circling High", cost: 1, leaning: "boldness",
        prerequisitesAnyOf: [["hit_and_gone"], ["blur_of_needles"], ["ambush_hive"]], delta: { range: { max: 3 } } },
      { id: "wing_shear", name: "Wing Shear", cost: 1, prerequisites: ["circling_high"], leaning: "boldness", delta: { jamCooldownTicks: 8 } },
      { id: "high_pass", name: "High Pass", cost: 2, prerequisites: ["wing_shear"], leaning: "boldness",
        delta: { range: { max: 4 }, hits: { min: 3, max: 3 }, power: -6 },
        note: "LANE H NOTABLE. Three strafing passes from out of reach instead of one committed dive. Stay above it. Reach and height, at the cost of ever being in its face." },
      { id: "gliding", name: "Gliding", cost: 1, prerequisites: ["high_pass"], leaning: "boldness", delta: { accuracy: 10, cooldownTicks: -1 } },
      { id: "gone_before_it_turns", name: "Gone Before It Turns", cost: 2, leaning: "boldness",
        prerequisitesAnyOf: [["wingbeat"], ["gliding"]],
        delta: { critCooldownReset: true, situationalBonus: { condition: "targetStatused", multiplier: 1.4 } },
        note: "KEYSTONE. Reachable from either lane. It is still turning around when the next pass lands." },

      // ================= SOCIABILITY: the hive =================
      { id: "swarm_signal", name: "Swarm Signal", cost: 1, leaning: "sociability", delta: { rallyCall: { ticks: 60 } },
        note: "OPENER. Splits into a marking lane and a sustaining lane. Marks the stung target; the whole hive's independently-run targeting converges on it." },
      // --- Lane M: the mark.
      { id: "hive_tempo", name: "Hive Tempo", cost: 1, leaning: "sociability",
        prerequisitesAnyOf: [["swarm_signal"], ["ambush_hive"], ["venom_mark"]], delta: { cooldownTicks: -1 } },
      ...
      { id: "tending_drones", name: "Tending Drones", cost: 1, leaning: "sociability",
        prerequisitesAnyOf: [["swarm_signal"], ["ambush_hive"], ["venom_mark"]],
      ...
      { id: "drone_relay", name: "Drone Relay", cost: 2, prerequisites: ["close_formation"], leaning: "sociability",
        ...
        note: "LANE S NOTABLE. No mark at all — the hive does not converge, it sustains. Every drone that fights beside another comes back stronger." },
      { id: "one_mind", name: "One Mind", cost: 1, prerequisites: ["drone_relay"], leaning: "sociability",
        delta: { positionSwap: true, positionSwapPull: 1 },
        note: "Drones trade places mid-flurry. Shipped primitive, used by exactly one node in the whole roster (Peck's Snatch and Swap)." },
      { id: "nothing_forgets", name: "Nothing Forgets", cost: 2, leaning: "sociability",
        prerequisitesAnyOf: [["no_stragglers"], ["one_mind"]],
        delta: { shape: { kind: "burst", radius: 1 }, hitsArea: true, statusSpreads: true, power: -8 },
        note: "KEYSTONE. Reachable from either lane. The swarm stops being individuals and arrives as a cloud." },
      ...
      { id: "blur_of_needles", name: "Blur of Needles", cost: 2, prerequisites: ["faster_pass"], leaning: "aggression",
        ...
        note: "BRIDGE NOTABLE. Escalates the crosslink's own tempo lever, and drops you at the head of EITHER lane in both branches it connects." },
      ...
      { id: "ambush_hive", name: "Ambush Hive", cost: 2, prerequisites: ["deeper_cover"], leaning: "sociability",
        ...
        note: "BRIDGE NOTABLE. The mark it sets comes from somewhere nothing was looking." },
      ...
      { id: "venom_mark", name: "Venom Mark", cost: 2, prerequisites: ["deeper_marking"], leaning: "aggression",
        ...
        note: "BRIDGE NOTABLE. Its own venom-reliability lever escalated until the mark spreads with it." },
    ]),
  },
